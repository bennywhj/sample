...
class UVDisparity
{
  public:
    ...
    inline void SetCalibPars(CalibPars& calib_par){calib_ = calib_par;}
    inline void SetROI3D(ROI3D& roi_3d){roi_ = roi_3d;}
    ...
    inline void SetOutThreshold(double out_th){out_th_ = out_th;}  // set outlier threshold
    inline void SetInlierTolerance(int inlier_tolerance){inlier_tolerance_ = inlier_tolerance;} //set inlier tolerance
    inline void SetMinAdjustIntense(int min_adjust_intense){min_adjust_intense_= min_adjust_intense;} //set minimum adjust intensity for segmentation
